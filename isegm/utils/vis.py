...
def create_vis_images(image, mask, clicks_list=None, pos_color=(0, 255, 0), neg_color=(0, 0, 255), radius=4):
    """
    Generates three separate images for visualization:
    1. A binary segmentation mask (black and white).
    2. A map of positive clicks on a black background.
    3. A map of negative clicks on a black background.

    Args:
        image (np.array): The original image (used for getting dimensions).
        mask (np.array): The predicted segmentation mask (boolean or 0/1).
        clicks_list (list): A list of click objects.
        pos_color (tuple): BGR color for positive clicks.
        neg_color (tuple): BGR color for negative clicks.
        radius (int): Radius of the drawn clicks.

    Returns:
        tuple: (binary_mask_vis, pos_clicks_vis, neg_clicks_vis)
    """
    h, w, _ = image.shape

    # 1. Create the binary segmentation mask visualization
    # The mask is expected to be boolean or 0/1. We convert it to a 3-channel BGR image.
    binary_mask_vis = (mask.astype(np.uint8) * 255)[:, :, np.newaxis]
    binary_mask_vis = np.concatenate([binary_mask_vis] * 3, axis=2)

    # 2. & 3. Create images for positive and negative clicks
    pos_clicks_vis = np.zeros_like(image)
    neg_clicks_vis = np.zeros_like(image)

    if clicks_list is not None and len(clicks_list) > 0:
        pos_points = [click.coords for click in clicks_list if click.is_positive]
        neg_points = [click.coords for click in clicks_list if not click.is_positive]

        # Draw positive clicks in green on a black background
        pos_clicks_vis = draw_points(pos_clicks_vis, pos_points, pos_color, radius=radius)

        # Draw negative clicks in red on a black background
        neg_clicks_vis = draw_points(neg_clicks_vis, neg_points, neg_color, radius=radius)

    return binary_mask_vis, pos_clicks_vis, neg_clicks_vis


# --- END: Add new code to vis.py ---
