...
from ..utils.serialization import get_config_repr
...
class ISTrainer(object):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            with torch.no_grad():
                ...
                for click_indx in range(num_iters):
                    ...
                    gate = 0 if click_indx == 0 else 1

                    net_input = torch.cat((image, prev_output), dim=1) if self.model_cfg['with_prev_mask'] else image

                    prev_output = torch.sigmoid(self.net(net_input, points, gate=gate)['instances'])

                    new_click_order = 100 + click_indx + 1
                    points = get_next_points(prev_output, gt_mask, points, new_click_order)

                    if not validation:
                        self.net.train()

                if self.model_cfg['with_prev_mask'] and self.prev_mask_drop_prob > 0 and num_iters > 0:
                    zero_mask = np.random.random(size=prev_output.size(0)) < self.prev_mask_drop_prob
                    prev_output[zero_mask] = torch.zeros_like(prev_output[zero_mask])

            final_gate = 1 if num_iters > 0 else 0

            batch_data['points'] = points

            net_input = torch.cat((image, prev_output), dim=1) if self.net.with_prev_mask else image
            output = self.net(net_input, points, gate=final_gate)
    ...
    def find_next_n_points(self, image, gt_mask, points, prev_output,num_points, eval_mode=False, grad=False):
    ...
    def _get_points_for_first_correction(self, points):
        batch_size = points.shape[0]
        device = points.device
        points_for_correction = torch.full_like(points, -1.0, device=device)
        for i in range(batch_size):
            valid_indices = torch.where(points[i, :, -1] >= 0)[0]
            if len(valid_indices) > 0:
                first_valid_idx = valid_indices[0]
                points_for_correction[i, 0, :2] = points[i, first_valid_idx, :2]
                points_for_correction[i, 0, -1] = 0
        return points_for_correction

    @property
    def is_master(self):
        return self.rank == 0

def get_next_points(pred, gt, points, click_order, pred_thresh=0.49):
    ...
    for bindx in range(fn_mask.shape[0]):
        ...
        inner_mask = dt > dt.max() / 2.0
        indices = np.argwhere(inner_mask)
        if len(indices) > 0:
            coords = indices[np.random.randint(0, len(indices))]

            order = float(click_order)

            if is_positive:
                empty_locs = torch.where(points[bindx, :num_points, 2] < 0)[0]
                loc = empty_locs[0] if len(empty_locs) > 0 else num_points - 1

                points[bindx, loc, 0] = float(coords[0])
                points[bindx, loc, 1] = float(coords[1])
                points[bindx, loc, 2] = order
            else:
                empty_locs = torch.where(points[bindx, num_points:, 2] < 0)[0]
                loc = empty_locs[0] + num_points if len(empty_locs) > 0 else 2 * num_points - 1
