...
def modulate_prevMask(prev_mask, points, R_max):
    with torch.no_grad():
        ...
        coord_rows, coord_cols = torch.meshgrid(row_array, col_array, indexing='ij')  # 保持与numpy一致的索引方式
        ...
        for bindx in range(points.shape[0]):
            ...
            if last_point[bindx, :, -1] == 1:
                ...
                L2_diff[modWindow] = (L2_diff[modWindow] / (L2_diff[modWindow].max() + 1e-8)) * 1000
                diff_th = L2_diff[modWindow].median()
                exp = -(max_gamma - 1) / (diff_th ** 3 + 1e-8) * (L2_diff[modWindow] - diff_th) ** 3 + 1
                exp[exp <= 1] = 1
